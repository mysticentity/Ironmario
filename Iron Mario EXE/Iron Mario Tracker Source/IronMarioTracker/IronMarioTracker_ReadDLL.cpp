...
uintptr_t GetModuleBaseAddress(DWORD pid, const wchar_t* moduleName) {
    ...
    if (hSnapshot == INVALID_HANDLE_VALUE) {
        MessageBoxA(NULL, "Failed to create snapshot!", "Error", MB_OK | MB_ICONERROR);
        return 0;
    }
    ...
    std::string moduleList = "Loaded Modules:\n";
    if (Module32First(hSnapshot, &moduleEntry)) {
        do {
            std::string modName = WideToString(moduleEntry.szModule);
            uintptr_t baseAddr = (uintptr_t)moduleEntry.modBaseAddr;

            moduleList += modName + " -> 0x" + std::to_string(baseAddr) + "\n";

            if (!_wcsicmp(moduleEntry.szModule, moduleName)) {
                moduleBaseAddress = baseAddr;
            }
    ...
    // Show all modules in a message box
    MessageBoxA(NULL, moduleList.c_str(), "Modules Debug", MB_OK | MB_ICONINFORMATION);

    return moduleBaseAddress;
}


int ReadIntegerFromMemory(uintptr_t offset) {
    DWORD pid = GetParaLLEl64PID();
    if (pid == 0) {
        MessageBoxA(NULL, "Failed to find ParaLLEl64 process!", "Error", MB_OK | MB_ICONERROR);
        return -1;
    }

    HANDLE hProcess = OpenProcess(PROCESS_VM_READ, FALSE, pid);
    if (hProcess == NULL) {
        MessageBoxA(NULL, "Failed to open ParaLLEl64 process.", "Error", MB_OK | MB_ICONERROR);
        return -1;
    }

    uintptr_t moduleBase = GetModuleBaseAddress(pid, L"parallel_n64_next_libretro.dll");
    if (moduleBase == 0) {
        MessageBoxA(NULL, "Module base address is NULL! The DLL might not be loaded.", "Error", MB_OK | MB_ICONERROR);
    ...
    // Debugging: Show computed addresses
    std::string debugMsg = "PID: " + std::to_string(pid) +
        "\nModule Base: 0x" + std::to_string(moduleBase) +
        "\nComputed Address: 0x" + std::to_string(address);
    MessageBoxA(NULL, debugMsg.c_str(), "Memory Debug", MB_OK | MB_ICONINFORMATION);

    // Check if the memory is valid before reading
    MEMORY_BASIC_INFORMATION mbi;
    if (VirtualQueryEx(hProcess, (LPCVOID)address, &mbi, sizeof(mbi)) == 0) {
        MessageBoxA(NULL, "VirtualQueryEx failed! Address is invalid or not mapped.", "Error", MB_OK | MB_ICONERROR);
        CloseHandle(hProcess);
        return -1;
    }

    // Debugging: Show memory protection info
    std::string protectMsg = "Memory Base: 0x" + std::to_string((uintptr_t)mbi.BaseAddress) +
        "\nRegion Size: " + std::to_string(mbi.RegionSize) +
        "\nMemory State: " + std::to_string(mbi.State) +
        "\nMemory Protect: " + std::to_string(mbi.Protect);
    MessageBoxA(NULL, protectMsg.c_str(), "Memory Protection Info", MB_OK | MB_ICONINFORMATION);

    // Ensure the memory is readable
    if (!(mbi.Protect & PAGE_READWRITE || mbi.Protect & PAGE_READONLY)) {
        MessageBoxA(NULL, "Memory is not readable!", "Error", MB_OK | MB_ICONERROR);
        CloseHandle(hProcess);
        return -1;
    }
    ...
    if (!ReadProcessMemory(hProcess, (LPCVOID)address, &value, sizeof(value), &bytesRead) || bytesRead != sizeof(value)) {
        MessageBoxA(NULL, "Failed to read memory at computed address!", "Error", MB_OK | MB_ICONERROR);
        CloseHandle(hProcess);
        return -1;
    }
...
// Memory Addresses
struct MEM {
    static uintptr_t GetCurrentSongID() {
        uintptr_t moduleBase = GetModuleBaseAddress(GetParaLLEl64PID(), L"parallel_n64_next_libretro.dll");

        if (moduleBase == 0) {
            MessageBoxA(NULL, "Failed to get module base address!", "Error", MB_OK | MB_ICONERROR);
            return 0;
        }

        uintptr_t offset = 0x638A438;  // Explicitly set correct offset
        uintptr_t computedAddress = moduleBase + offset;

        // Debugging output
        std::string debugMsg = "Module Base: 0x" + std::to_string(moduleBase) +
            "\nOffset: 0x" + std::to_string(offset) +
            "\nFinal Computed Address: 0x" + std::to_string(computedAddress);
        MessageBoxA(NULL, debugMsg.c_str(), "Computed Address Debug", MB_OK | MB_ICONINFORMATION);
    ...
    static constexpr uintptr_t CURRENT_SONG_ID = 0x7FFC30FAA438;
...
void renderOverlay() {
    ...
    while (window.isOpen()) {
        ...
        uintptr_t songAddress = MEM::GetCurrentSongID();
        int currentSongID = ReadIntegerFromMemory(songAddress);
        // Debugging: Show the retrieved song ID in a message box
        std::string debugMessage = "Current Song ID from memory: " + std::to_string(currentSongID);
        MessageBoxA(NULL, debugMessage.c_str(), "Debug Info", MB_OK | MB_ICONINFORMATION);
