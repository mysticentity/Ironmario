...
#include <thread>
#include <ctime>
#include <json.h> // JSON handling library
#include <windows.h>
...
DWORD GetProject64PID() {
    ...
    if (Process32First(hSnapshot, &pe)) {
        do {
            ...
            if (processName == "Project64.exe") {  // Adjust if your emulator has a different name
...
void renderOverlay() {
    ...
    std::string fontPath = "C:\\Windows\\Fonts\\consolas.ttf";  // Full system path
    ...
    while (window.isOpen()) {
        ...
        state.currentSongID = readMemory(MEM::CURRENT_SONG_ID);
        std::string songName = (state.currentSongID != -1 && CONFIG::MUSIC_DATA::SONG_MAP.count(state.currentSongID)) ? CONFIG::MUSIC_DATA::SONG_MAP.at(state.currentSongID) : "Unknown Song";
        sf::Text songText("Song: " + songName, font, 18);
        songText.setPosition(10, 100);
        songText.setFillColor(sf::Color::Cyan);
    }
    ...
    int main() {
        std::cout << "IronMarioTracker starting..." << std::endl;
        renderOverlay();
        ...
        std::cout << "Starting IronMario Tracker..." << std::endl;
        renderOverlay();
        std::cout << "Overlay should now be running..." << std::endl;
